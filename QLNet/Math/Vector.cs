    ...
    */
    public struct Vector {
        ...
        public Vector(int size, double value, double increment) : this(size) {
            for (int i = 0; i < n_; i++)
                data_[i] = value += increment;
        }

        public Vector(ref Vector from) {
            data_ = from.data_;
            n_ = from.n_;
        }
        ...
        public static bool operator ==(Vector to, Vector from) {
            ...
            for (int i = 0; i < from.n_; i++)
                if (from.data_[i] != to.data_[i]) return false;
        ...
        private static Vector operVector(ref Vector v1, ref Vector v2, Func<double, double, double> func) {
            if (v1.n_ != v2.n_)
                throw new ApplicationException("operation on vectors with different sizes (" + v1.n_ + ", " + v2.n_);

            Vector temp = new Vector(v1.n_);
            for (int i = 0; i < v1.n_; i++)
                temp.data_[i] = func(v1.data_[i], v2.data_[i]);
            return temp;
        }
        private static Vector operValue(ref Vector v1, double value, Func<double, double, double> func) {
            Vector temp = new Vector(v1.n_);
            for (int i = 0; i < v1.n_; i++)
